...
const { getScreenPixelsPerCSSPixel } = require('sdk/window/utils.js');
...
class Recommender {
  ...
  showPanel(button) {
    ...
    const panelView = viewFor(this.panel);
    const buttonView = viewFor(button);
    const window = button.ownerDocument.defaultView;

    panelView.style.margin = '';

    // Open the popup by the anchor.
    const rect = buttonView.getBoundingClientRect();
    const zoom = getScreenPixelsPerCSSPixel(window);
    const screenX = rect.left + window.mozInnerScreenX * zoom;
    const screenY = rect.top + window.mozInnerScreenY * zoom;

    // Set up the vertical position of the popup relative to the anchor
    // (always display the arrow on anchor center)
    let horizontal = null;
    let vertical = null;
    if (screenY > window.screen.availHeight / 2 + this.panel.height) {
      vertical = 'top';
    } else {
      vertical = 'bottom';
    }

    if (screenX > window.screen.availWidth / 2 + this.panel.width) {
      horizontal = 'left';
    } else {
      horizontal = 'right';
    }
